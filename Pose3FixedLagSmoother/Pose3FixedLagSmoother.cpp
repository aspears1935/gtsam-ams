...
int main(int argc, char** argv) {
  ...
  for(double time = deltaT; time <= 3.0; time += deltaT) {
    ...
    Pose3 currentPose(Rot3::ypr(0.0, 0.0, 0.0), Point3(time * 2.0, 0.0, 0.0));
    newValues.insert(currentKey, currentPose);

    // Add odometry factors from two different sources with different error stats
    Pose3 odometryMeasurement1 = Pose3(Rot3::ypr(0.02, 0.0, 0.0), Point3(0.61, -0.08, 0.0));
    //0.05 rads in rpy and 0.1m in xyz
    noiseModel::Diagonal::shared_ptr odometryNoise1 = noiseModel::Diagonal::Variances((Vector(6) << 0.05, 0.05, 0.05, 0.1, 0.1, 0.1).finished());
    newFactors.push_back(BetweenFactor<Pose3>(previousKey, currentKey, odometryMeasurement1, odometryNoise1));

    Pose3 odometryMeasurement2 = Pose3(Rot3::ypr(0.01, 0.0, 0.0), Point3(0.47, 0.03, 0.0));
    noiseModel::Diagonal::shared_ptr odometryNoise2 = noiseModel::Diagonal::Variances((Vector(6) << 0.05, 0.05, 0.05, 0.05, 0.05, 0.05).finished());
    newFactors.push_back(BetweenFactor<Pose3>(previousKey, currentKey, odometryMeasurement2, odometryNoise2));
